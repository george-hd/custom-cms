<div class="options-container">
    ...
    <div>
        ...
        if($model['list_categories']) {
        ?>
        <label for="category-filter" class="margl20">Filter by parent</label>
        <select id="category-filter" class="language">
            <option value="">All</option>
            <?php
            foreach ($model['list_categories'] as $category) {
                ?>
                <option value="<?php echo $category->cat_id ?>"<?php echo($category->cat_id == $model['current_filter'] ? 'selected' : ''); ?>><?php echo ucfirst($category->cat_val); ?></option>
                <?php
            }
            ?>
        </select>
